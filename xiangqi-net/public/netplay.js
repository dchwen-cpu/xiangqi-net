...
(function(){
  ...
  var aiSeat=null, aiLevel=6, aiDriving=false;   // AI 坐哪方、棋力、是否由我驱动
  ...
  function init(){
    ...
    socket.on('move', function(m){
      ...
      if     (m.idx===appliedCount){ applyRemote(m); appliedCount++; onMoveMade(m.by, socket); refreshTurn(); setTimeout(function(){ maybeRunAI(socket); },400); }
    ...
    socket.on('table:players', function(p){
      ...
      if(p.aiSeat!==undefined){
        aiSeat=p.aiSeat; aiLevel=p.aiLevel||6;
        aiDriving = !!(aiSeat && myColor && ((myColor==='r'&&aiSeat==='black')||(myColor==='b'&&aiSeat==='red')));
      }
    });
    socket.on('table:over',    function(o){ stopTimer(); setStatus(o.reason||'对局结束',true); showReturnButton(); });
  ...
  function setupHooks(socket){
    ...
    // 引擎只在"我驱动AI且轮到AI"时才允许出手，其余一律拦截
    if(typeof triggerAI==='function'&&!triggerAI.__net){
      var _t=triggerAI;
      window.triggerAI=function(){
        if(window.__netMode && !window.__aiTurnNow) return;   // 非AI回合，不许引擎动
    ...
    if(typeof makeMove==='function'&&!makeMove.__net){
      ...
      window.makeMove=function(fr,fc,tr,tc){
        var isAiMove = !!window.__aiTurnNow;          // 这一步是不是AI下的
        if(!isAiMove) { try{ aiThinking=false; }catch(e){} }
        if(window.__netMode&&isSpectator&&!applyingRemote) return;
        // 本地手动走子：必须轮到自己
        if(window.__netMode&&!applyingRemote&&!isAiMove&&myColor&&typeof turn!=='undefined'&&turn!==myColor) return;
        var r=_m(fr,fc,tr,tc);
        if(window.__netMode&&!applyingRemote){
          appliedCount++;
          socket.emit('move',{fr:fr,fc:fc,tr:tr,tc:tc});
          var side = isAiMove ? aiSeat : (myColor==='r'?'red':'black');
          window.__aiTurnNow=false;
          onMoveMade(side, socket);
          refreshTurn();
          setTimeout(function(){ maybeRunAI(socket); },300);   // 我走完，看看该不该让AI应招
        }
        return r;
      };
  ...
  // 若轮到AI且由我驱动，就让本地引擎算一步
  function maybeRunAI(socket){
    if(!aiDriving || !aiSeat) return;
    if(typeof turn==='undefined') return;
    var aiTurnChar = aiSeat==='red' ? 'r' : 'b';
    if(turn!==aiTurnChar) return;
    if(window.__aiTurnNow) return;             // 已在算
    try{
      window.__aiTurnNow = true;
      aiLevelApply(aiLevel);
      setStatus('立山AI 思考中…');
      if(typeof triggerAI==='function') triggerAI();
    }catch(e){ window.__aiTurnNow=false; }
  }
  function aiLevelApply(lv){
    try{
      aiLevel = lv;
      if(typeof window.aiLevel!=='undefined') window.aiLevel = lv;
  ...
  function doSync(st, socket){
    ...
    setPlayers(st.seats||{});
    refreshTurn();
    setTimeout(function(){ maybeRunAI(socket); },600);   // 开局若AI先手，让它先走
  }
  ...
  function showReturnButton(){
    if(document.getElementById('net-return-btn')) return;
    var btn=document.createElement('a');
    btn.id='net-return-btn';
    btn.href='../';
    btn.textContent='返回棋室 →';
    btn.style.cssText='position:fixed;top:50%;left:50%;transform:translate(-50%,calc(-50% + 60px));'
      +'z-index:2147483647;background:#9d2c21;color:#f2e8d5;text-decoration:none;'
      +'padding:12px 32px;border-radius:6px;font:1rem "Kaiti SC","楷体",serif;'
      +'letter-spacing:.15em;box-shadow:0 4px 16px rgba(0,0,0,.35)';
    document.body.appendChild(btn);
  }
