...
(function(){
  ...
  function init(){
    ...
    socket.on('move', function(m){
      ...
      if     (m.idx===appliedCount){ applyRemote(m); appliedCount++; onMoveMade(m.by, socket); refreshTurn(); setTimeout(function(){ maybeRunAI(); },400); }
    ...
    socket.on('table:over',    function(o){ stopTimer(); setStatus(o.reason||'对局结束',true); showEndPanel(o.reason); });
